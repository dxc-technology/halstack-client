...
describe ("Adding links to empty HAL resource", function() {testAddLinks(); testAddLinkWithoutHref()})
describe ("Adding title to empty HAL resource", function() {testAddTitle()});
describe ("Adding items to empty HAL resource", function() {testAddItem(); testAddItems()});
describe ("Adding properties to empty HAL resource", function() {testAddSchemaProperties()});
describe ("Adding interaction to empty HAL resource", function() {testAddInteraction()});
describe ("Adding interaction to non-empty HAL resource", function() {testAddInteraction(HalResource(colJsonObjRes))});
describe ("Adding options to empty HAL resource", function() {testAddOptions()});
...
function testAddSchemaProperties(resource=HalResource()) {
  resource.addPropertiesSchema(schemaProperties);
  test('Adding properties :', () =>{
    expect(resource.getSchemaProperties().length).toBe(3);
  });
  test ('Checking property type: ', () =>{
    expect(resource.getSchemaProperties()[0].type).toBe('string');
  });
};
