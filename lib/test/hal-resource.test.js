...
describe("Adding required Properties to options", function () {
  testAddOptionsRequired();
});
...
function testItem(itemHalRes) {
  ...
  test(`How many properties? ${itemHalRes.getProperties().length}`, () => {
    expect(itemHalRes.getProperties().length).toBe(6);
  });
...
function testAddOptionsRequired(){
  let copyUserResource = {...{}, ...userResource};
  delete copyUserResource._options.required;
  let halUserResource = HalResource(copyUserResource);
  halUserResource.addRequiredProperties(['user_name', 'client_name']);
  test("Expect the resource to have 2 required properties:", () => {
    expect(halUserResource.getRequiredProperties().length).toBe(2);
  });

  halUserResource = HalResource(copyUserResource);
  halUserResource.addRequiredProperty('user_name');
  halUserResource.addRequiredProperty(['client_name']);
  test("Expect the resource to have 1+1 required properties:", () => {
    expect(halUserResource.getRequiredProperties().length).toBe(2);
  });
