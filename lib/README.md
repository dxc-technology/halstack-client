...
const { HalResource, HalResponse, HalApiCaller } = require("@dxc-technology/halstack-client");
```
...
```js
// require from the dependency the modules you need
import { HalResource, HalResponse, HalApiCaller } from "@dxc-technology/halstack-client";
```
...
| `options`          | `function:Promise(response)`    | Receives a { url, headers } object as a parameter. Returns a promise with the options.                                        |
